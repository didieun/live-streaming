...
} from "./styles/StreamingNotificationsStyle";
...
function StreamingNotifications(props) {
    ...
    return (
        ...
        >
            ...
            {rows.length !== 0 ?
                rows.map((row, i) => (
                    <BoxStyle key={i} select={row.new} onClick={row.new ? handleClickInviteToStreamDialogOpen :handleClickNoticeDialog}>
                        {row.notice &&
                            <NoticeBox>
                        ...
                        <TitleText select={row.new}>{row.title}</TitleText>
...
export default StreamingNotifications;
